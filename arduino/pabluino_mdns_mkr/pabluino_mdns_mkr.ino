...
// const IPAddress serverIp(10, 0, 1, 5); // 192, 168, 0, 140
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    //if you write 'c' in the command line, it will connect to the server at the specified IP address above
    if (incomingByte == 'c') {
      connectToServer();
      delay(50);
    }
...
void connectToServer() {
  ...
  if (serverIp) {
    ...
    Serial.println("Error, no server IP!");
  }
}
